...
public:
    AES(int keyLen = 256);
    // AES(int keyLen = 256, uint16_t ploynomial = 0x11b);
    // 电码本模式
    ...
    // 计数器模式
    unsigned char *EncryptCTR(unsigned char in[], unsigned int inLen, unsigned char key[], unsigned char *iv, unsigned int &outLen);
    unsigned char *DecryptCTR(unsigned char in[], unsigned int inLen, unsigned char key[], unsigned char *iv);
    ...
    void printSBox(uint8_t sbox[][0x10]);


    void genSBox(uint16_t ploynomial, uint8_t add = 0x63);//常量的使用使得在S盒中没有不动点

    void genInvSBox(uint16_t ploynomial , uint8_t add = 0x5);//常量的使用使得在S盒中没有反不动点
